...
import { getUsageDataById, updateUsageDataById } from 'apiSdk/usage-data';
...
function UsageDataEditPage() {
  ...
  const handleSubmit = async (values: UsageDataInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateUsageDataById(id, values);
      mutate(updated);
      resetForm();
