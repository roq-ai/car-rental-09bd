...
import { getUsageDataById } from 'apiSdk/usage-data';
import { UsageDataInterface } from 'interfaces/usage-data';

function UsageDataViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<UsageDataInterface>(
    () => (id ? `/usage-data/${id}` : null),
    () =>
      getUsageDataById(id, {
        relations: ['vehicle'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('usage_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/usage-data/edit/${id}`} passHref legacyBehavior>
                    <Button
