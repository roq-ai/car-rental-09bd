...
import { getUsageData } from 'apiSdk/usage-data';
import { UsageDataInterface } from 'interfaces/usage-data';

import { UsageDataListPage as ShowCaseList } from 'pages/usage-data';
...
function DashboardPage() {
  ...
  const usageDataStat = useSWR<PaginatedInterface<UsageDataInterface>>(
    () => `/usage-data`,
    () => getUsageData({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Usage Data',
      result: usageDataStat?.data?.totalCount,
      isLoading: usageDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`usage-data`} passHref legacyBehavior>
                  <Button
